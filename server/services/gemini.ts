...
import { generateQuestionsWithDeepSeek } from "./deepseek.js";

const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY || "" });

function getDecodedText(str: string): string {
  if (!str) return "";
  // Check if string is likely base64 (no spaces, alphanumeric, ends with = or matches length rules)
  const isBase64 = /^[A-Za-z0-9+/]+={0,2}$/.test(str.trim()) && str.length % 4 === 0 && !str.includes(" ");
  if (isBase64) {
    try {
...
export async function generateInterviewQuestions(
  ...
  try {
    // Decode base64 data to text only if it is actually base64
    const decodedJobDescription = getDecodedText(jobDescription);
    const decodedResume = getDecodedText(resume);
...
export async function analyzeFeedback(
  ...
  try {
    // Decode base64 job description only if it is base64
    const decodedJobDescription = getDecodedText(jobDescription);
